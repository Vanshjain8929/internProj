...
document.getElementById('newTopicForm').addEventListener('submit', async function (e) {
    ...
    const fileInput = document.getElementById('fileUpload');
    const fileData = await convertFilesToBase64(fileInput.files);
    ...
    const storedTopics = JSON.parse(localStorage.getItem('cghsTopics') || '[]');
    storedTopics.unshift(formData);
    localStorage.setItem('cghsTopics', JSON.stringify(storedTopics));

    // ✅ NEW: Store the latest topic separately (instead of using long URL)
    localStorage.setItem('cghsLastTopic', JSON.stringify(formData));

    showSuccessMessage('Topic created successfully! Redirecting...');
    setTimeout(() => {
        window.location.href = 'cghs.html'; // 👈 No query string now
    }, 1500);
});
...
function saveDraft() {
    ...
    localStorage.setItem('cghsTopicDraft', JSON.stringify(draft));
...
function logout() {
    ...
    setTimeout(() => {
        window.location.href = 'cghs.html';
    }, 1000);
}

function loadDraft() {
    const savedDraft = localStorage.getItem('cghsTopicDraft');
    if (savedDraft) {
        try {
